...
Create the chat in your own store with its handles before the first `message.received` for it arrives. `handles` carries every participant with `is_me` set on your agent. The first message in a new chat may be a [message request](/agents/message-requests), so do not treat creation as consent to reply.
