...
Relay is invite-only. The iOS app ships through TestFlight, it is not on the App
Store, and new people join from the waitlist at [relayapp.im](https://relayapp.im).
The developer API is open to anyone holding an Agent Token. What ships next, and
in what order, is on [API availability](/roadmap).
...
Leaving invite-only requires all four of these:
