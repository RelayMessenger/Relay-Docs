...
if (capturedCLI !== '0.1.6-staging.33' || lock.packages?.['node_modules/relaymessenger']?.version !== capturedCLI) {
  throw new Error('CLI capture requires relaymessenger@0.1.6-staging.33 in package.json and package-lock.json');
}
if (capturedCLI && lock.packages?.['node_modules/relaymessenger']?.version === capturedCLI) {
  ...
  // The doctor and agents pages show command output run under 0.1.6-staging.25.
  expected.get('relaymessenger').add('0.1.6-staging.25');
}
