...
def validate_behavior(root):
    ...
    receipts = read('concepts/messages/receipts.mdx')
    ...
    uploads = read('concepts/messages/attachments.mdx')
    ...
    imports = read('concepts/messages/import-media.mdx')
    require('Public URL import', imports, r'10 MiB \(10,485,760 bytes\)', r'DNS|resolved address', r'redirect', r'private', r'credentials')
    downloads = read('concepts/messages/receiving-media.mdx')
    require('Download attachments', downloads, r'relay\.attachments\.retrieve', r'60 minutes', r'Range:', r'206', r'416')
    require('Delete attachments', read('concepts/messages/delete-attachments.mdx'), r'relay\.attachments\.delete', r'owner', r'409', r'404')
    require('Attachment types', read('concepts/messages/attachment-types.mdx'), r'any (?:file type|valid MIME type)', r'application/octet-stream', r'nosniff', r'Content-Disposition', r'SVG', r'100 MiB \(104,857,600 bytes\)')

    signing = read('events/webhooks/verify-signatures.mdx')
    ...
    receiver = read('events/webhooks/index.mdx')
    require('Webhook receiver', receiver, r'commit.*before.*2xx', r'event_id', r'503', r'204')
    subscriptions = read('events/webhooks/subscriptions.mdx')
    ...
    delivery = read('events/webhooks/delivery.mdx')
    ...
    socket = read('events/websocket/index.mdx')
    ...
    frames = read('events/websocket/protocol.mdx')
    ...
    observe = read('events/websocket/observe-events.mdx')
    require('Non-consuming observer', observe, r'observe=true', r'observational', r'ack', r'full_sync_complete', r'reject')

    require('Typing', read('concepts/chats/typing.mdx'), r'chat\.typing_indicator\.started', r'chat\.typing_indicator\.stopped', r'every 60 seconds', r'90 seconds')
    limits = read('live/rate-limits.mdx')
    require('Chat limits', limits, r'Other Contacts in `to` \| 6', r'Total active Contacts \| 7')
    membership = read('concepts/chats/participants.mdx')
    ...
    idempotency = read('live/idempotency.mdx')
