...
def validate_behavior(root):
    ...
    receipts = read('build/messages/receipts.mdx')
    ...
    uploads = read('build/messages/attachments.mdx')
    ...
    imports = read('build/messages/import-media.mdx')
    require('Public URL import', imports, r'10 MiB \(10,485,760 bytes\)', r'DNS|resolved address', r'redirect', r'private', r'credentials')
    downloads = read('build/messages/receiving-media.mdx')
    require('Download attachments', downloads, r'relay\.attachments\.retrieve', r'60 minutes', r'Range:', r'206', r'416')
    require('Delete attachments', read('build/messages/delete-attachments.mdx'), r'relay\.attachments\.delete', r'owner', r'409', r'404')
    require('Attachment types', read('build/messages/attachment-types.mdx'), r'any (?:file type|valid MIME type)', r'application/octet-stream', r'nosniff', r'Content-Disposition', r'SVG', r'100 MiB \(104,857,600 bytes\)')

    signing = read('build/events/verify-signatures.mdx')
    ...
    receiver = read('build/events/webhooks.mdx')
    require('Webhook receiver', receiver, r'commit.*before.*2xx', r'event_id', r'503', r'204')
    subscriptions = read('build/events/subscriptions.mdx')
    ...
    delivery = read('build/events/delivery.mdx')
    ...
    socket = read('build/events/websocket.mdx')
    ...
    frames = read('build/events/websocket/protocol.mdx')
    ...
    observe = read('build/events/websocket/observe-events.mdx')
    require('Non-consuming observer', observe, r'observe=true', r'observational', r'ack', r'full_sync_complete', r'reject')

    require('Typing', read('build/chats/typing.mdx'), r'chat\.typing_indicator\.started', r'chat\.typing_indicator\.stopped', r'every 60 seconds', r'90 seconds')
    limits = read('build/live/rate-limits.mdx')
    require('Chat limits', limits, r'Other Contacts in `to` \| 6', r'Total active Contacts \| 7')
    membership = read('build/chats/participants.mdx')
    ...
    idempotency = read('build/live/idempotency.mdx')
