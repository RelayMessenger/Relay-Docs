...
def validate_behavior(root):
    ...
    require('Unadd lifecycle', requests, r'Unadd keeps the existing conversation in Chats',
            r'next incoming message from your agent makes the conversation a message request',
    ...
    uploads = read('messages/attachments.mdx')
    ...
    imports = read('messages/import-media.mdx')
    require('Public URL import', imports, r'10 MiB \(10,485,760 bytes\)', r'DNS|resolved address', r'redirect', r'private', r'credentials')
    downloads = read('messages/receiving-media.mdx')
    require('Download attachments', downloads, r'relay\.attachments\.retrieve', r'60 minutes', r'Range:', r'206', r'416')
    require('Delete attachments', read('messages/delete-attachments.mdx'), r'relay\.attachments\.delete', r'owner', r'409', r'404')
    require('Attachment types', read('messages/attachment-types.mdx'), r'any (?:file type|valid MIME type)', r'application/octet-stream', r'nosniff', r'Content-Disposition', r'SVG', r'100 MiB \(104,857,600 bytes\)')

    signing = read('webhooks/verify-signatures.mdx')
    ...
    receiver = read('webhooks/index.mdx')
    require('Webhook receiver', receiver, r'commit.*before.*2xx', r'event_id', r'503', r'204')
    subscriptions = read('webhooks/subscriptions.mdx')
    ...
    delivery = read('webhooks/delivery.mdx')
    ...
    socket = read('websocket/index.mdx')
    ...
    frames = read('websocket/protocol.mdx')
