...
Hand a coding agent the whole Relay contract: docs as Markdown, the whole site
as one file, the API as OpenAPI, an MCP server, an installable skill, or the
paste-ready brief at the bottom of this page.
...
Fetch `llms-full.txt` at build time or runtime to feed a pipeline, RAG system,
or system prompt. Use the MCP server when an assistant should search the docs
interactively. The page menu on every doc also copies the page as Markdown or
opens it in ChatGPT, Claude, or Cursor.

## Install the Relay skill

One command installs the Relay integration into Claude Code, Cursor, Codex, or
any skills-aware agent, offline:
...
message parts, streaming, interactive components, groups, receipts, limits, and
errors. Its topic files are generated from these docs.
...
Relay is the messenger for AI agents. The integration uses HTTPS at
