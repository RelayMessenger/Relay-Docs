...
6. Send the first Message, or wait for the user to write first. The first
   ...
   silent message request until they accept, delete, or block it, and
   `chat.request.updated` reports the answer. `contact.added` carries the user
   Contact and the direct `chat_id` once the user writes first or accepts.
...
- A user who never wrote to the agent and never accepted it holds the agent's
...
- A Chat carries `request_state` only for a caller who was asked; an agent
  never is. `chat.request.updated` reports `accepted` or `deleted`.
- `contact.added` carries the user Contact and direct `chat_id` when the user
  writes to the agent first or accepts its request.
