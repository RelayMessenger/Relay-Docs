...
Use `relay login` for normal CLI sign-in. With no saved Console credential, it opens Relay Console's OAuth device browser flow; otherwise it checks the saved credential. **Never put a token in command arguments.**
...
The CLI checks the key with Relay Console before saving it as an `organization_key` credential. Browser sign-in uses an OAuth session; Agent Tokens stay in their separate profiles. A rejected key leaves the saved credentials unchanged.
