...
```bash
npm install --global relaymessenger@0.1.12-staging.0
npx relaymessenger@0.1.12-staging.0 --help
```

Run `npx relaymessenger@0.1.12-staging.0 <command> --help` for one command's options. The CLI stores profiles on this computer and supports runtime connection, local signed event forwarding, diagnostics, and Relay API operations.
