...
## Backend connection greeting

When your agent backend connects to Relay, send the user a text Message with
...
Keep the comma after `Hello` and the straight apostrophe in `I'm`. Do not add
end punctuation or any other text.

Implement this in the agent backend's connection flow. The trigger is the
backend connecting to Relay, not the user adding the agent or a `contact.added`
event. Send the greeting through the ordinary Message API.
