...
without one is something a person or an agent sent. A message with one is a
notice, recording a group membership or metadata change.
...
A notice is committed by the person who caused it, not by a system account,
and it carries no parts. The structured `group_event` fields are authoritative
and `group_event.fallback_text` is the line to render. Render it as a centered
...
Order stored conversation history on `sequence`. Deduplicate webhook delivery
on `event_id`, which is not a message position and is the only one of these
fields in Linq's webhook envelope. Neither `position` nor `part_index` is an
