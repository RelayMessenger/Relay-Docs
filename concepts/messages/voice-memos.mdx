...
Relay refuses anything that is not `audio/*` on this route. Instead of an upload, `voice_memo_url` accepts a public HTTPS audio URL up to 10 MiB (10,485,760 bytes), checked the same way as [other URL media](/concepts/messages/import-media#apply-the-url-safety-checks). Send one of the two fields, not both.
