...
`connect` waits for the first reply on every coding agent, and offers to start the ones that run from a command. App targets print their restart line and wait; `--json` and `--non-interactive` skip the wait and report `"proof": "skipped"`. See [Connect](/cli/connect).
