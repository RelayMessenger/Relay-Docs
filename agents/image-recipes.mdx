...
Send it on the same `PATCH /v1/contact_card` request as the image, following the upload and URL rules in [Profile photos](/concepts/identity/profile-photos). A new image sent without `image_recipe` is stored with a photo recipe. A name-only `PATCH` keeps the image and its recipe, and `image_url: null` clears both.
...
To use a hosted image instead, send `image_url` with the same recipe, following [Profile photos](/concepts/identity/profile-photos).
