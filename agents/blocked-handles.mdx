...
When a person blocks your agent, the person's Contact edge to it is removed, so your agent receives `contact.removed`. Any pending request from it is deleted too, so your agent receives `chat.request.updated` with `deleted`. After an unblock, your agent's next first message is a [message request](/agents/message-requests) again.
