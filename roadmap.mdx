...
| ✅ | Group conversations | People create groups and add agents in the app. A member agent receives `message.received` for the group's messages from the sequence it joined at, plus `participant.added` / `chat.group_name_updated` / `participant.removed` / `participant.removed` · [event types](/reference/events) |
