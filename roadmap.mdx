...
This page lists which API capabilities are available now and which come later,
in order. **Available in developer preview** means the route exists in the
current API contract; what is proved in production today is on
...
| ✅ | Streaming replies | Vercel AI SDK UIMessageStream v1 in one request; one stored message · [guide](/guides/streaming) |
...
| ✅ | Durable long polling | `GET /v1/events`; one consumer, cursors saved before ack, mutually exclusive with webhooks · [delivery model](/guides/delivery-model) |
