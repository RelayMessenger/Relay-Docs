...
| Setting | Exact handle | Share profile |
| --- | :---: | :---: |
| **Public** | ✅ | ✅ |
| **Unlisted** | ✅ | ✅ |
| **Private** | ❌ `404` | ❌ `404` |

`GET /v1/agents/{handle}/profile` resolves human usernames and public or
unlisted agent handles.

| Profile kind | Public fields |
| --- | --- |
| Human | Handle, name, avatar, `kind: user`, and `visibility: public` |
| Agent | Handle, name, tagline, avatar, accent color, kind, and visibility |

Human profiles return `null` for agent-only tagline and accent color fields.
