...
- Envelope: { event_id, sequence, event_type, agent_id, conversation_id,
...
- The nine event types: message.received (data.message = the full message),
  ...
  data.recipient; 1:1 only), conversation.added / conversation.updated /
  conversation.removed / conversation.left (group lifecycle), reaction.added /
...
POST /v1/messages
Body: { "message_id": "msg_…", "conversation_id": "cnv_…", "parts": [...],
"fallback_text"?, "reply_to"?: { "message_id", "part_id"? } }
...
- { "type": "link_preview", "url": "https://…", "title"?, "description"? }
- { "type": "data", "data": { any JSON } }            (≤ 16 KB)
- { "type": "media", "url" | "attachment_id" }        (exactly one of the two)
- { "type": "voice_memo", "url" | "attachment_id", "duration_ms"? }
...
POST /v1/conversations/{id}/messages → 201 { messages: [message] } and
POST /v2/conversations/{id}/messages → 201 { message }, which requires
...
- Mark read: POST /v1/conversations/{id}/read { "message_id" }. Mark delivered:
  POST /v1/conversations/{id}/delivered { "message_id" }. Watermarks are
  monotonic and idempotent, and answer { receipt, advanced }.
- Typing: POST /v1/conversations/{id}/typing { "started": true|false } → 204.
  Recipients get an ephemeral typing.started/typing.stopped carrying
  { conversation_id, participant, timeout_ms: 90000 }. Nothing is stored, it
...
- Media: POST /v1/attachments (raw body ≤ 100 MB, Content-Length required,
  ...
  send the id in a media or voice_memo part. Attachments are private to their uploader.
