...
For `call`, `actor` is the caller and `subject` is the callee. The marker exists from placement and projects the current Call state. `system_event.call` carries these fields and is `null` on every other system event type.
...
| `answered_at`, `ended_at` | string or null | ISO 8601 timestamps, or `null` until the corresponding transition. |
| `from` | object | Caller Contact. |
| `to` | array | One callee Contact. |
| `end_reason` | string or null | `completed`, `declined`, `canceled`, `no_answer`, `disconnected`, or `failed`. `null` until the call ends. |
| `connected` | boolean | Whether the call was answered. |
| `duration_seconds` | integer or null | Whole seconds from answer to end. `null` while unfinished or if never answered. |
