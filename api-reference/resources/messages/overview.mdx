...
A system message has `is_system_message: true`. Its `system_event.type` is `chat_created`, `participant_added`, `participant_removed`, `group_name_updated`, `group_icon_updated`, `contact_card_shared`, or `call_ended`.

For `call_ended`, `actor` is the caller and `subject` is the callee. `system_event.call` carries these fields and is `null` on every other system event type.

| Field | Type | Description |
| --- | --- | --- |
| `id` | string | ID of the ended call. |
| `mode` | string | `audio`. |
| `end_reason` | string | `completed`, `declined`, `canceled`, `no_answer`, `disconnected`, or `failed`. |
| `connected` | boolean | Whether the call connected. |
| `duration_seconds` | integer or null | Call duration in seconds. |
