...
The marker exists from the moment the call is placed; `call.status` and `call.end_reason` carry its state.
...
| `mode` | string | `audio`. |
| `status` | string | `ringing`, `active`, or `ended`. |
...
| `end_reason` | string or null | `null` until the call ends; then `completed`, `declined`, `canceled`, `no_answer`, `disconnected`, or `failed`. |
| `connected` | boolean | Whether the call was answered. |
