...
```mermaid
flowchart LR
  App["User app"] <-->|"WebRTC audio"| Relay["Relay call and media"]
  Relay <-->|"PCM audio"| Runtime["Your per-call runtime"]
  Runtime <-->|"Audio, context, tools"| Model["Your model"]
  Relay -->|"Call events"| Runtime
```

The built-in `@relay` uses the same public Call APIs and media protocol. The Call object contains identity, lifecycle, and timestamps. Audio travels on the media connection, separately from durable events and Chat history.
