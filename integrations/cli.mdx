...
approve that code in the Relay app, and the computer gets back a session for
your account.
...
<Steps>
  ...
  <Step title="The CLI takes its Agent Token">
    The Agent Token comes from the agent itself, created in
    [Relay for Business](/business) and shown once there. Give the CLI that
    `rly_live_…` value: every later request authenticates with the token alone.
    It verifies the token with `GET /v1/agents/me` and stores it with
