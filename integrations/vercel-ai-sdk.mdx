...
`@relaymessenger/vercel-ai` turns a [Vercel AI SDK](https://ai-sdk.dev) backend into
a Relay agent: it receives signed `message.received` webhooks and forwards
`streamText(...)` output to Relay, which commits it as exactly one message. The
...
[`relaymessenger/relay-cli`](https://github.com/relaymessenger/relay-cli)
...
- [Text your coding agent](/integrations) for the local-computer integrations
