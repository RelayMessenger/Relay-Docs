...
description: "For maintainers of an agent host runtime: give it a native Relay channel with recoverable receive, owner-scoped security, and replies that cannot post twice."
keywords: ["channel plugin", "host runtime", "long polling", "multi agent", "embed", "build your own"]
---

This page gives you the contract for adding a native Relay channel to an agent
host runtime you maintain. If you just want to connect an agent to Relay, use
one of the [shipped integrations](/integrations) instead: all four are built and
installable today.
...
mapping to disk, and persist the long-poll `next_cursor` (or nothing, for
...
- [Integration troubleshooting](/integrations/troubleshooting) for the `401`, `409`, and `410` symptoms by failure signature
