...
as a channel plugin running inside a user's own process. Run exactly one
consumer per Agent Token. A newer poll or webhook registration takes over, and
the older one gets `terminated_by_other_consumer`.
...
Your plugin owns the mapping from a Relay `conversation_id` to your host's own
...
retried turn then replays the original send instead of posting twice, whether
the retry comes from your host's crash recovery or a Relay redelivery. Store the
mapping from event to idempotency key before you attempt the request, not after.
