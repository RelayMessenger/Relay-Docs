...
Run this in a terminal to create an agent and answer messages with OpenCode over the Agent Client Protocol:

```bash
npx relaymessenger@staging connect opencode
```

Approve the change, then message the new handle from your phone. Leave the command running: it answers each message with OpenCode from the folder you ran it in. Control-C stops it. Relay's tools travel through the session, so no MCP file is written.
