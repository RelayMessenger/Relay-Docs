...
<Steps>
  ...
  <Step title="Set the webhook receiver">
    ...
    curl -sS -X PUT "https://api.relayapp.im/v1/webhook" \
      ...
      -d '{"url": "https://<your-worker>.workers.dev/messengers/relay/webhook"}'
    ```
...
<Steps>
  ...
  <Step title="Set the webhook receiver">
    ...
    curl -sS -X PUT "https://api.relayapp.im/v1/webhook" \
      -H "Authorization: Bearer $RELAY_AGENT_TOKEN" \
      -H "Content-Type: application/json" \
      -d '{"url": "https://relay-agent.<your-subdomain>.workers.dev/webhooks/relay"}'
    ```
...
| `RELAY_WEBHOOK_SECRET` | `signing_secret` from `PUT /v1/webhook`. Relay never returns it again |
