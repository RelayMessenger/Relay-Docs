...
description: "Run a Relay agent as a Cloudflare Agent: one agent instance per conversation, a SQLite ledger, and an alarm that survives eviction."
keywords: ["cloudflare", "cloudflare agents", "agents sdk", "durable objects", "workers", "starter", "template", "wrangler", "webhook", "deploy"]
---
...
- **A Cloudflare account.** The starter deploys with `wrangler`.
- **Node 20 or newer**, if you scaffold locally rather than using the deploy button.
