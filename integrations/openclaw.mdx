...
text. OpenClaw keeps the runtime, and Relay becomes one of its messaging
channels.

Relay ships this as a native plugin rather than an ACP engine preset, because the
plugin runs inside OpenClaw's gateway lifecycle and uses OpenClaw's own routing,
session, and outbound APIs.
