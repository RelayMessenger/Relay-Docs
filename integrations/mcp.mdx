...
[Invoice](/interactions/invoice). Use `search_docs` with `invoice`, then `execute` to send a Message whose only part is the invoice, with a Stripe checkout link. Only a verified agent can send one. `client.messages.invoice.update` moves its status.
