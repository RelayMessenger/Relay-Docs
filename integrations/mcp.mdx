...
[Selection](/interactive-components/selection). Use `search_docs` with `selection` and verbose detail, then `execute` with text and selection parts through the SDK. Message and event results preserve `selected_values` and `reply_to` beside bullet text.
