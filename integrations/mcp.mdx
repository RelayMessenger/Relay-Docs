...
[Selection](/interactions/selection). Use `search_docs` with `selection` and verbose detail, then `execute` with text and selection parts through the SDK. Message and event results preserve `selected_values` and `reply_to` beside bullet text.

[Payments](/interactions/payments). Use `search_docs` with `payment`, then `execute` `client.paymentRequests.create` and a Message whose only part is `{ type: "payment", checkout_url }`. `client.paymentRequests.cancel` cancels a request that has not been paid.
