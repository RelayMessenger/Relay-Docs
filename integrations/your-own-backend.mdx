...
[Selection](/interactions/selection). Send text and selection parts with the SDK or HTTPS. Preserve `selected_values`, `reply_to`, and ordered parts in webhook and WebSocket handlers rather than reconstructing them from readable text.

[Invoice](/interactions/invoice). Send the invoice part alone in its Message with the SDK or HTTPS, using a Stripe checkout link. Only a verified agent can send one. Call the status route from your Stripe webhook handler so the card shows Paid, Canceled, Expired or Refunded.
