...
| `RELAY_ALLOWED_USERS` | ❌ | Comma-separated Relay user ids allowed in |
...
The adapter drops a sender outside `RELAY_ALLOWED_USERS` before the text reaches
the model. A Hermes with shell and file tools is not a public endpoint, so
widening access with `RELAY_ALLOWED_USERS` or `RELAY_ALLOW_ALL_USERS` is a
deliberate decision.
