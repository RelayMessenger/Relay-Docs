...
<Steps>
  ...
  <Step title="Text the agent">
    ...
    agent typing while it works, posts one final reply, and delivers supported
    tool permission requests as owner-only messages. The ACP bridge attaches
    one-tap reply suggestions; the channel plugin takes a typed
    `yes <id>` or `no <id>` reply.
  </Step>
</Steps>
