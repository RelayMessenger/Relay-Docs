...
| Event consumer | The agent must have no webhook registered |

<Warning>
Relay allows one event consumer per Agent Token. A webhook and this integration
cannot share an agent: whichever registers last takes the stream, and the other
receives `terminated_by_other_consumer`. Give the bridge its own agent when a
webhook backend already serves one.
</Warning>
...
| One session per agent | A second Claude session on the same agent fails closed instead of stealing the event stream from the first. Point the second session at a different Relay agent. |
