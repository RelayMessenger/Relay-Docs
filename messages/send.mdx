...
The response is the stored message. Keep its `id` if you will reply to, react to, edit, or unsend it. Text and media parts can be mixed in any order, but two text parts may not sit next to each other; [message parts](/messages/parts) has the rules for each part type.
