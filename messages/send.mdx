...
<Note>
The app draws the inline syntax below in received text parts and hides the delimiters; the chat list row and the push banner show the same words. Block syntax such as headings, lists, and fenced code is shown as written. A text part that carries mentions is shown as written, since mention ranges index the source.
</Note>
