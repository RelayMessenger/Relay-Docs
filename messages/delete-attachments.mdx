...
Clearing a [group photo](/chats/group-photo#clear-the-photo) changes the current photo, but the history event that set it can still reference the old Attachment, so clearing alone does not make deletion succeed. Taking back a message is a separate operation: [unsend it](/messages/edit-and-unsend#unsend-a-message) first, which removes its Attachment references, then delete.
...
- [Unsend a message](/messages/edit-and-unsend#unsend-a-message)
