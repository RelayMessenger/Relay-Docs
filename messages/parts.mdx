...
Each item has a text `label` and exactly one of `id` or `url`. An `id` button comes back to you when tapped. A `url` button opens the page in the user's browser and sends nothing.
