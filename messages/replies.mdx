...
The person sent a chart as a message of its own. Send the reply through either [send path](/messages/send) with `reply_to` inside `message`, naming that message and its part 0:

<Tabs>
  <Tab title="Preview">
    <ChatPreview scene="replies" label="The agent's reply joined to the person's chart by the reply line" json={{"message": {"parts": [{"type": "text", "value": "That image looks correct."}], "reply_to": {"message_id": "01993d50-b4ce-71e6-8e65-35d325d95ddb", "part_index": 0}}}} />
  </Tab>
  <Tab title="JSON">
    ```json
    {
