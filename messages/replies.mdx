...
Ordinary replies can target text, media (including voice memos), link, and `button_reply` parts. A button tap uses `button_reply` to target the originating `buttons` part.
...
| `422` | [2006](/api-reference/errors#2006) | An ordinary reply targets a buttons part. | Reply to the accompanying text or the tap reply bubble. |
