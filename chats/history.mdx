...
A call has one [`call` system event](/api-reference/resources/messages/overview#system-events) with the caller as `actor`, the callee as `subject`, and its state in `system_event.call`. The marker exists from the moment the call is placed; `call.status` and `call.end_reason` carry its state.
