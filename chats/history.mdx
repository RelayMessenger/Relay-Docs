...
An ended call leaves one [`call_ended` system event](/api-reference/resources/messages/overview#system-events) with the caller as `actor`, the callee as `subject`, and the outcome in `system_event.call`.
