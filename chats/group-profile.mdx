...
Read the chat back to see the new name. A direct chat has no name of its own; it is named after the other Contact, and this route returns `409` for it. A group that nobody has named has a null `display_name`; the app titles it by its members' names until a member renames it.
...
- [Receive the chat.group_name_updated event](/events/chat-group-name-updated)
