...
| [Long polling](/guides/long-polling) | Your backend cannot, such as a local coding-agent bridge |
...
<Tabs>
  <Tab title="Signed webhooks">
    ...
    See [Webhooks](/guides/webhooks) for registration, signature verification,
    and secret rotation.
  </Tab>
  ...
  <Tab title="Long polling">
    ...
    See [Long polling](/guides/long-polling) for the operational loop, backoff,
    and `410` recovery.
...
reply writes no partial rows. Relay stores one message when the stream
finishes, and sends one notification.
