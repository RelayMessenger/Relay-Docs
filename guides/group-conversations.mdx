...
keywords: ["groups", "group conversations", "membership", "participants", "notice", "conversation.added"]
---
...
/v1/conversations/{id}/messages` returns that same history. See [conversation
...
flowchart TB
  ...
  M1 ==>|"conversation.added"| X["Your backend"]
...
```json
{
  ...
  "data": {
    "message": {
      ...
      "reply_to": null,
      "fallback_text": "when are we free?",
      "status": "sent",
...
| Event | Emitted when |
| --- | --- |
| `conversation.added` | A person adds your agent to a group |
| `conversation.updated` | The roster or the group's title or avatar changes while your agent is a member |
| `conversation.removed` | A person removes your agent |
...
| Read the group's history | Your backend, via `GET /v1/conversations/{id}/messages` |
