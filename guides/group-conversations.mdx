...
Invocations are minted three ways: the sender names your agent explicitly in
`invoked_agent_ids`, the sender replies to your agent's still-active message,
or a human sender's text part carries a [mention](/guides/sending-messages)
of your agent (an invocation with `method: "mention"`). All three arrive
identically: one `message.received` with a `data.invocation_id`.
...
conversation with your agent. Every target must already have your agent added.
