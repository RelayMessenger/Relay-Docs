...
description: "Send ordered parts with POST /v1/messages, reply to a message, and make retries safe."
...
`reply_to` targets a message by id, and optionally one exact part of it:
...
`part_id` from the part you are answering. Relay freezes a quote of the target
when the reply commits, so an edit to the target later changes the reply's
`quote_state` rather than the words it quoted.

A legacy `part_index` is accepted here and is translated to that part's
`part_id` before anything is stored. Send `part_id`.
...
## Send one message with /v2

`POST /v2/conversations/{conversation_id}/messages` commits the whole send as
one message. Text and media stay together as ordered parts instead of being
split into a run of messages, and every part comes back with a permanent
`part_id` that replies, reactions, and edits address.
...
The client mints `message_id` before queueing the send, and it is both the
canonical id of the message and the retry key. There is no `Idempotency-Key`
header on `/v2`.

| Retry | Result |
| --- | --- |
| Same id, same request | `201` replaying the original message |
| Same id, different content | `409 idempotency_conflict` |
| Same id, after the message was unsent | `201` replaying the tombstone |

Mint the id with any ULID generator that emits lowercase Crockford base32, and
mint it once per logical send so a retry after a lost response resolves to the
same message.

| Difference from `/v1` | `/v2` |
| --- | --- |
| Messages committed | Exactly one |
| Retry key | `message_id` in the body |
| `reply_to` | `part_id` only. `part_index` answers `422` |

`/v1` keeps splitting sends into content runs for the clients already shipped
against it, and both wires read and write the same rows. See
[Compatibility and migration](/concepts/message-model#compatibility-and-migration).

## Next steps

- [Message model](/concepts/message-model) for part ids, edits, and unsend
