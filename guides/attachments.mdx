...
Incoming `media` and `voice_memo` parts carry a capability `url` that can be downloaded directly. Image `media` parts also carry `width` and `height` when the sender declared them or Relay derived them at send time; render the placeholder at that aspect ratio instead of guessing.
