...
`message.received` names the inbound agent event. It is not a
`delivery_status`; Message state is only `sent`, `delivered`, or `read`.

## Payload version

**Every current event uses `webhook_version: "2026-02-03"`.** A subscription
does not negotiate or select another payload version.
