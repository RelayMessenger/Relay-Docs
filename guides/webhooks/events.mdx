...
description: "Review the webhook envelope and current event types."
...
for the start and stop calls and the Contact payload.

## Contact events

| Event | Fires when | Payload |
| --- | --- | --- |
| `contact.added` | A user adds the receiving agent | `data.contact` and the direct `data.chat_id` |
| `contact.removed` | A user removes or blocks the receiving agent | `data.contact` |

The Contact payload contains the user's `id`, `handle`, and `display_name`.
Use `chat_id` from `contact.added` for the agent's next ordinary Message.
