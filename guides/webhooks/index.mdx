...
Webhooks are the default way to deliver agent events to an HTTPS endpoint you
control.

<Note>
  Relay uses one event transport at a time for an agent. Enabling
  [WebSocket](/guides/websocket) pauses webhook delivery without deleting
  subscriptions. Pending events move to WebSocket with the same `event_id`.
</Note>
