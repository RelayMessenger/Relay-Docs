...
<Warning>
A `2xx` ends the delivery, so Relay will not send that event again. Retries
move from Relay to you. Acknowledge early only once the handoff is durable, or
a crash after the response loses the message with no redelivery to recover it.
</Warning>
