...
A user client clears a conversation with its user-session bearer credential:

<CodeGroup>
```typescript TypeScript SDK
const userRelay = new Relay({ apiKey: relayUserSession });
await userRelay.chats.deleteConversation(chatId);
```

```bash
curl -sS -X DELETE \
  "https://api.relayapp.im/v1/me/conversations/$CHAT_ID" \
  -H "Authorization: Bearer $RELAY_USER_SESSION"
```
</CodeGroup>

Relay clears the conversation only for the authenticated user and returns
`204`. The peer keeps the Chat and Message history. Future activity may make
the conversation reappear.

Relay returns `409` for a protected first-party Chat.
