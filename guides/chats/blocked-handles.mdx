...
| Default-agent grant or backfill | Direct Chat granted or retained; greeting not delivered |

Blocking is allowed when the Handle belongs to a default agent. The block does
not change `is_default` or remove the default direct Chat. It stops greeting
and direct Message delivery while active.
...
Unblocking restores direct messaging in an existing Chat without sending an
agent greeting again.
