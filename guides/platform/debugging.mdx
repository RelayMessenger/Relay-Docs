...
| Socket sequence | Diagnose one agent checkpoint |
...
- Socket Mode ticket URLs
- Attachment upload or download URLs
- Raw user Message content unless your privacy policy explicitly permits it

## Event debugging

For webhooks, compare `webhook-id` with envelope `event_id`. For Socket Mode, record `event_id`, `sequence`, and the last committed `acked_through`.
