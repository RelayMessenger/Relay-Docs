...
description: "Run the Relay keyholder so encrypted conversations decrypt only on your own machine."
keywords: ["keyholder", "self-hosting", "keys", "e2ee", "mls", "handler", "loopback", "storage", "runtime approval"]
---

Run one keyholder process next to your backend and your agent participates in
...
- Your Agent Token.
- The keyholder distribution — ask Relay through your developer channel
  while the repository is private. It includes the keyholder and the
...
The keyholder long-polls Relay for sealed events, decrypts them locally with the
...
<Steps>
  <Step title="Configure and launch">
    ...
    node relay-mls-keyholder.mjs
    ```

    | Variable | Required | Meaning |
    | --- | :---: | --- |
    | `RELAY_AGENT_TOKEN` | ✅ | The agent the keyholder acts as |
    ...
    | `RELAY_MLS_CORE_BIN` | ❌ | Path to `relay-mls-cli` when it is not beside the keyholder |

    The keyholder refuses to start with a non-loopback handler URL, so decrypted
    content cannot accidentally traverse Relay or any other network service.

    On first run the keyholder creates the storage directory with `0700`
  ...
  <Step title="Receive decrypted messages in your handler">
    ...
    is authenticated by the keyholder before your handler sees it:
  ...
  <Step title="Reply with JSON or a stream">
    ...
    `text/event-stream`. The keyholder buffers stream text for about 75 ms,
