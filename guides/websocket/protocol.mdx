...
The diagnostic `observe=true` connection adds `observational: true` to its
`ready` frame and uses a transient cursor. It rejects `ack` and
`full_sync_complete`; the real consumer retains the durable checkpoint.
See [read-only observation](/guides/websocket#observe-events-without-consuming).
...
**An `event` frame can carry any of Relay's 18 webhook event types.** See the
[complete event table](/guides/webhooks/events#event-types).
