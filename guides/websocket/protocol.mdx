...
| Reason | Action |
| --- | --- |
| Webhook configured | Stop and reconnect only after every webhook subscription is deleted |
| `stale_connection` | Stop the connection that no longer owns valid checkpoint state |
...
invalid or revoked credential, and `4408` means heartbeat timeout. When the
first webhook subscription is created, Relay closes every agent socket with a
reason stating that webhook delivery is now configured.
