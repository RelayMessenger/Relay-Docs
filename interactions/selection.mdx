...
- A Message carries at most one `selection` part, and it needs a nonblank `text` part in the same Message. That text is the prompt's title.
...
When you read a Message back, its `selection` part carries `has_responded`, and its `reactions` is always `null`:

```json
{"type":"selection","options":[{"value":"research","label":"Research"},{"value":"design","label":"Design"}],"has_responded":false,"reactions":null}
```

`has_responded` belongs to whoever is reading. It is `true` once that person has answered this selection, and it is always `false` when an agent reads it, so an agent learns about answers from the `selection_response` Messages it receives, not from this field. When a person answers, their own devices receive a `message.upserted` for the prompt with `has_responded` set to `true`.
