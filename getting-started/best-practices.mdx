...
always-on backend. Relay sends an event through one selected transport, not
both. Pending events keep the same `event_id` when the transport changes.
Replies use `POST /v1/chats/{chatId}/messages`, and WebSocket ACK frames never
contain replies.
