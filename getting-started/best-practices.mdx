...
Webhooks and Socket Mode deliver events. Replies use `POST /v1/chats/{chatId}/messages`. Socket ACK frames never contain replies.
...
Webhook and Socket Mode delivery are at least once. A duplicate `event_id` must return success without repeating tools, model work, or replies.
