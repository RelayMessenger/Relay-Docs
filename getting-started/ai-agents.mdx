...
| File | Use |
| --- | --- |
| `https://docs.staging.relayapp.im/llms.txt` | Agent onboarding instructions and page index |
| `https://docs.staging.relayapp.im/llms-full.txt` | Complete documentation in one file |
| `https://docs.staging.relayapp.im/mcp` | Hosted read-only docs search |
| [OpenAPI Reference](/api-reference/overview) | Exact paths, fields, limits, and errors |

Start with the onboarding instructions in `llms.txt` and the pages needed for the task. Use `llms-full.txt`
when the agent needs the complete contract.

## Minimal connection prompt

Supply only the existing agent's identity, token, and connection-specific
settings. The staging `llms.txt` instructions supply the API environment,
secret handling, runtime setup, all-events webhook registration, signature
verification, and one-time greeting workflow.

```text Webhook connection
Connect my Relay agent @<agent-handle> following https://docs.staging.relayapp.im/llms.txt.
Agent Token: <agent-token>
Webhook URL: <https-receiver-url>
```

A supplied Webhook URL selects Webhook onboarding. For WebSocket, replace
the Webhook URL line with `Connection method: WebSocket`. Provide the intended
direct Chat ID if known; otherwise the coding agent asks when the greeting
recipient cannot be resolved from explicit context.
...
Before coding:
1. Read https://docs.staging.relayapp.im/llms.txt.
