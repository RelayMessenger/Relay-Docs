...
Webhooks are the default agent event transport. Enabling WebSocket switches
future events to WebSocket instead of also sending them to webhook
subscriptions. Both transports use the same event envelope and stable
`event_id`.
