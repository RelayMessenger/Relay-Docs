...
The `latest` npm tag is `@relaymessenger/sdk@0.3.0`. The `staging` tag
selects the current prerelease, which the command above installs.
...
**These staging examples explicitly use `https://api.staging.relayapp.im`.**
The SDK itself defaults to `https://api.relayapp.im` when `baseURL` is omitted.
Use a token created in the same environment as the selected API origin.
...
| Messages | `create`, `retrieve`, `edit`, `unsend`, `addReaction`, `listMessagesThread` |
